...
class ReasoningTemplate(Template):
    ...
    def encode_oneturn(
        ...
        # LLM4ICD extension: enable_thinking is None means "extract the
        # final-turn <think> block, attach it to the prompt side (no loss),
        # and let the loss-bearing response begin after </think>". Used by the
        # ICD reasoning-SFT recipes so the model SEES the thought trace at
        # train time but learns to emit only the final answer.
        extracted_think: Optional[str] = None
    ...
    def encode_multiturn(
        ...
        # LLM4ICD extension (mirrors the encode_oneturn branch above):
        # when enable_thinking is None, extract every assistant turn's
        # <think> block and route it to the prompt side at encode time.
